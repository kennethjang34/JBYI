	...
	return (dispatch, getState) => {
		...
		axios
			...
			.then((response) => {
				...
				dispatch(loadFriendRequestsAction(username))
			});
	};
};
...
export const checkAuthAction = (dispatch, getState) => {
	...
	if (token !== undefined) {
		...
		if (localStorage.getItem("friends")) {
			...
			dispatch(loadFriendsAction(currentUser));
		}
		if (localStorage.getItem("friendRequests")) {
			dispatch(loadFriendRequests(JSON.parse(localStorage.getItem("friendRequests"))));
		} else {
			loadFriendsAction(currentUser)
		}
